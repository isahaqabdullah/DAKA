...
import type { AttendanceWorkflowStatus, ChipTone, Cohort, ScheduledClass, SyllabusStatus } from "../types";
import { T, syllabusTone, workflowTone } from "../theme";
...
import {
  Btn, EmptyState, FieldLabel, FieldShell, FormField, InlineSelect, PageHeader,
  SectionLabel, StatusChip, Surface, TableHeader, TableRow, TableShell, Td, Th,
  inputStyle, textareaStyle,
} from "./shared";
import { BookOpen, CalendarDays, Info, Plus, X } from "lucide-react";
...
export function AdminSchedulePage({ initialCohortId, onSelectCohort }: Props) {
  ...
  const [cohortInfoOpen, setCohortInfoOpen] = useState(false);
  ...
  const primaryActionLabel = tab === "sessions" ? "Add Session" : "Add Syllabus";
  ...
  const sessionPopupDescription = editingSessionId
    ...
    : "Confirm the schedule details for the new session.";
  const seriesCount = parsePositiveInteger(sessionDraft.seriesCount, 6);
  const seriesIntervalWeeks = parsePositiveInteger(sessionDraft.seriesIntervalWeeks, 1);
  ...
  const modeCards = [
    {
      id: "sessions",
      label: "Sessions",
      meta: `${sortedSessions.length}`,
      icon: <CalendarDays size={16} />,
    },
    {
      id: "syllabus",
      label: "Syllabus",
      meta: `${cohort.syllabus.length}`,
  ...
  return (
    <div style={{ display: "grid", gap: T.space3 }}>
      <PageHeader title="Schedule">
        <Btn onClick={() => tab === "sessions" ? openCreateSessionPopup() : setSyllabusPopupOpen(true)}>
          <Plus size={15} />
          {primaryActionLabel}
        </Btn>
      </PageHeader>
          ...
          <div style={{ display: "grid", gap: T.space2 }}>
            <div style={{ display: "flex", alignItems: "center", gap: T.space2, flexWrap: "wrap" }}>
              ...
              >
                {modeCards.map((mode) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ width: "22px", height: "22px", borderRadius: T.radiusSm, display: "grid", placeItems: "center", backgroundColor: isActive ? T.accentBg : T.surface, border: `1px solid ${isActive ? T.accentBorder : T.border}` }}>
                        {mode.icon}
                      </span>
              ...
              <Btn
                variant="secondary"
                size="compact"
                onClick={() => setCohortInfoOpen((current) => !current)}
                style={{ justifyContent: "center", minWidth: "96px", backgroundColor: T.surface, borderColor: T.border, color: T.heading }}
              >
                <Info size={14} />
                {cohortInfoOpen ? "Hide" : "Info"}
              </Btn>
            </div>
          ...
          {cohortInfoOpen ? (
            ...
            >
              {cohortDetails.map((detail) => (
                ...
                >
                  <span style={{ color: T.subtle, fontSize: T.textXs, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                    {detail.label}
                  </span>
                  <span style={{ color: T.heading, fontSize: T.textSm, fontWeight: 700, lineHeight: 1.35 }}>
                    {detail.value}
                  </span>
                </div>
              ))}
          ...
          <div style={{ display: "grid", gap: T.space3 }}>
            ...
            <TableShell>
              ...
              {cohort.syllabus.length === 0 ? (
                <EmptyState icon={<CalendarDays size={24} />} title="No syllabus entries" description="Add your first syllabus entry to the curriculum." action={<Btn onClick={() => setSyllabusPopupOpen(true)}>+ Add Syllabus</Btn>} />
      ...
      <CenterPopup
        ...
        footer={(
          <>
            ...
            <Btn type="submit" onClick={() => { const form = document.getElementById("syllabus-form") as HTMLFormElement | null; form?.requestSubmit(); }}>Save Syllabus</Btn>
          </>
        )}
      >
