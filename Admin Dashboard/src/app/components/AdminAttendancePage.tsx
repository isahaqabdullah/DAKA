...
export function AdminAttendancePage({ initialCohortId, initialClassId, onBackToDashboard, allowCohortSwitch = true, onSelectionChange }: AdminAttendancePageProps) {
  ...
  const [cohortStatusFilter, setCohortStatusFilter] = useState<CohortStatusFilter>("active");
  ...
  if (selectableCohorts.length === 0) {
    return (
      <div style={{ display: "grid", gap: T.space4 }}>
        <PageHeader title="Attendance" leading={backButton} />
                  ...
                  {visibleSessionOptions.map((option) => {
                    ...
                    return (
                      <TableRow key={option.session.id} columns={sessionCols} onClick={() => chooseSession(option.session.id)}>
                        ...
                        <Td bold>{option.session.topic || "Session"}</Td>
